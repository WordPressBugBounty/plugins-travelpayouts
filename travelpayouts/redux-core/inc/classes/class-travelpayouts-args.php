...
/**
 * Redux_Travelpayouts Framework Args Class
 *
...
if (! class_exists('Redux_Travelpayouts_Args', false)) {

    /**
     * Class Redux_Travelpayouts_Args
     */
    class Redux_Travelpayouts_Args
    {
        /**
        ...
        /**
         * TravelpayoutsSettingsFramework object.
         *
         * @var null
         */
        private $parent = null;
        ...
        /**
         * Switch to omit social icons if dev_mode is set to true and Redux_Travelpayouts defaults are used.
         *
         * @var bool
         */
        ...
        /**
         * Redux_Travelpayouts_Args constructor.
         *
        ...
        public function __construct($parent, $args)
        {
            ...
            $default = apply_filters('redux_travelpayouts/pro/args/defaults', $default);

            $args = Redux_Travelpayouts_Functions::parse_args($args, $default);

            $args = $this->args($args);
            $args = $this->default_cleanup($args);

            if (! in_array($args['font_display'], [ 'block', 'swap', 'fallback', 'optional' ], true)) {
            ...
            if ('Redux_Travelpayouts_extensions_demo' !== $args['opt_name'] && 'Redux_Travelpayouts_demo' !== $args['opt_name']) {
        ...
        {
            /**
             * Filter 'redux_travelpayouts/args/{opt_name}'
             *
             * @param     array     $args TravelpayoutsSettingsFramework configuration
             */

            // phpcs:ignore WordPress.NamingConventions.ValidHookName
            $args = apply_filters("redux_travelpayouts/args/{$args['opt_name']}", $args);

            /**
             * Filter 'redux_travelpayouts/options/{opt_name}/args'
             *
             * @param     array     $args TravelpayoutsSettingsFramework configuration
             */

            // phpcs:ignore WordPress.NamingConventions.ValidHookName
            $args = apply_filters("redux_travelpayouts/options/{$args['opt_name']}/args", $args);
        ...
        {
            if ($args['dev_mode'] || true === Redux_Travelpayouts_Helpers::is_local_host()) {
                if (! empty($args['admin_bar_links'])) {
                    foreach ($args['admin_bar_links'] as $idx => $arr) {
                        if (is_array($arr) && ! empty($arr)) {
                            foreach ($arr as $x => $y) {
                                if (strpos(Redux_Travelpayouts_Core::strtolower($y), 'redux') !== false) {
                                    $msg = '<strong>' . esc_html__(
                                        'Redux_Travelpayouts Framework Notice',
                                        'redux-framework'
                                    ) . ' </strong>' . esc_html__(
                                        'There are references to the Redux_Travelpayouts Framework support site in your config\'s ',
                                        'redux-framework'
