...
/**
 * Redux_Travelpayouts WordPress Data Class
 *
 * @class   Redux_Travelpayouts_WordPress_Data
 * @version 3.0.0
...
if (! class_exists('Redux_Travelpayouts_WordPress_Data', false)) {

    /**
     * Class Redux_Travelpayouts_WordPress_Data
     */
    class Redux_Travelpayouts_WordPress_Data extends Redux_Travelpayouts_Class
    {
        /**
        ...
        /**
         * Redux_Travelpayouts_WordPress_Data constructor.
         *
         * @param mixed $parent TravelpayoutsSettingsFramework pointer or opt_name.
         */
         ...
         */
        public function get($type = false, $args = [], $current_value = '', $ajax = false)
        {
            $opt_name = $this->opt_name;

            /**
             * Filter 'redux_travelpayouts/options/{opt_name}/pre_data/{type}'
             *
             * @param string $data
             */
            $pre_data = apply_filters("redux_travelpayouts/options/{$opt_name}/pre_data/$type", null); // phpcs:ignore WordPress.NamingConventions.ValidHookName
            if (null !== $pre_data || empty($type)) {
                return $pre_data;
            }

            if (isset($args['data_sortby']) && in_array($args['data_sortby'], [ 'value', 'key' ], true)) {
                $data_sort = $args['data_sortby'];
                unset($args['data_sortby']);
            } else {
                $data_sort = 'value';
            }
            if (isset($args['data_order']) && in_array($args['data_order'], [ 'asc', 'desc' ], true)) {
            ...
            $current_data = [];
            if (empty($current_value) && ! Redux_Travelpayouts_Helpers::is_integer($current_value)) {
                $current_value = null;
            } else {
            ...
            /**
             * Filter 'redux_travelpayouts/options/{opt_name}/data/{type}'
             *
             * @param string $data
             */

            // phpcs:ignore WordPress.NamingConventions.ValidHookName
            $data = apply_filters("redux_travelpayouts/options/{$opt_name}/data/$type", $data);
         ...
         * @return array
         */
        private function process_results($results = [], $id_key = '', $name_key = '', $add_key = true, $secondary_key = 'slug')
        {
            $data = [];
         ...
         * @return array
         */
        private function order_data($data = [], $sort = 'value', $order = 'asc')
        {
            if ('key' === $sort) {
                if ('asc' === $order) {
                    ksort($data);
        ...
        private function get_data($type, $args, $current_value)
        {
            ...
            switch ($type) {
                ...
                case 'font-icon':
                case 'font-icons':
                    $fs    = Redux_Travelpayouts_Filesystem::get_instance();
                    $fonts = $fs->get_contents(Redux_Travelpayouts_Core::$dir . 'assets/css/vendor/elusive-icons.css');
                    if (! empty($fonts)) {
                    ...
                    /**
                     * Filter 'redux_travelpayouts/font-icons'
                     *
                    ...
                    // phpcs:ignore WordPress.NamingConventions.ValidHookName
                    $font_icons = apply_filters('redux_travelpayouts/font-icons', $data);

                    /**
                     * Filter 'redux_travelpayouts/{opt_name}/field/font/icons'
                     *
                     * @param array $font_icons array of elusive icon classes
                     *
                     * @deprecated
                    ...
                    // phpcs:ignore WordPress.NamingConventions.ValidHookName
                    $font_icons = apply_filters("redux_travelpayouts/{$opt_name}/field/font/icons", $font_icons);
         ...
         */
        private function maybe_get_translation($type, &$current_value = '', $args = [])
        {
            switch ($type) {
                case 'categories':
                case 'category':
        ...
        private function maybe_translate(&$value, $post_type)
        {
            ...
            // phpcs:ignore WordPress.NamingConventions.ValidHookName
            $value = apply_filters("redux_travelpayouts/options/{$this->opt_name}/wordpress_data/translate/post_type_value", $value, $post_type);
            ...
            if (function_exists('icl_object_id')) {
                if (has_filter('wpml_object_id')) {
                    if (Redux_Travelpayouts_Helpers::is_integer($value)) {
            ...
            switch ($type) {
                ...
                case 'posts':
                    if (! empty($current_value)) {
                        $args['post__in'] = is_array($current_value) ? $current_value : [ $current_value ];
                    }
                    break;
         ...
         * @return array
         */
        private function get_arg_defaults($type, $args = [])
        {
            // In this section we set the default arguments for each data type.
            switch ($type) {
                case 'categories':
