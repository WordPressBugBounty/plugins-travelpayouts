...
/**
 * Redux_Travelpayouts Filesystem Class
 *
 * @class Redux_Travelpayouts_Filesystem
 * @version 4.0.0
...
if (! class_exists('Redux_Travelpayouts_Filesystem', false)) {
    ...
    class Redux_Travelpayouts_Filesystem
    {
        ...
        public function __construct($force_no_fs = false)
        {
            ...
            $this->cache_folder = trailingslashit($uploads_dir['basedir']) . 'redux_travelpayouts/';
        ...
        public function ftp_form()
        {
            if (isset($this->parent->ftp_form) && ! empty($this->parent->ftp_form)) {
                ...
                echo '<strong>' . esc_html__('File Permission Issues', 'redux-framework') . '</strong><br/>' . sprintf(esc_html__('We were unable to modify required files. Please ensure that %1$s has the proper read-write permissions, or modify your wp-config.php file to contain your FTP login credentials as %2$s.', 'redux-framework'), '<code>' . esc_url(Redux_Travelpayouts_Functions_Ex::wp_normalize_path(trailingslashit(WP_CONTENT_DIR)) . '/uploads/') . '</code>', ' <a href="https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants" target="_blank">' . esc_html__('outlined here', 'redux-framework') . '</a>');
                echo '</p>';
                echo '</div>';
                echo '<h2></h2>';
                echo '</div>';
        ...
        /**
         * Generates the default Redux_Travelpayouts cache folder.
         *
         * @return void
         */
        private function generate_default_files()
        {
            ...
            if (TRAVELPAYOUTS_DEBUG && ! $this->is_dir(Redux_Travelpayouts_Core::$dir)) {
                $this->mkdir(Redux_Travelpayouts_Core::$dir);
            }

            $hash_path = trailingslashit(Redux_Travelpayouts_Core::$dir) . 'hash';
            if (TRAVELPAYOUTS_DEBUG && ! $this->file_exists($hash_path)) {
                $this->put_contents($hash_path, Redux_Travelpayouts_Helpers::get_hash());
            }

            $version_path = trailingslashit(Redux_Travelpayouts_Core::$dir) . 'version';
            if (TRAVELPAYOUTS_DEBUG && ! $this->file_exists($version_path)) {
                $this->put_contents($version_path, Redux_Travelpayouts_Core::$version);
            } else {
                $version_compare = $this->get_contents($version_path);
                if (TRAVELPAYOUTS_DEBUG && Redux_Travelpayouts_Core::$version !== $version_compare) {
                    $this->put_contents($version_path, Redux_Travelpayouts_Core::$version);
                }
            }
        }
        ...
        private function create_index($path)
        {
            ...
            if (TRAVELPAYOUTS_DEBUG && ! $this->file_exists($index_path)) {
        ...
        {
            $panel = new Redux_Travelpayouts_Panel($this);
            return $panel->get_template($file);
        }

    }

    Redux_Travelpayouts_Filesystem::get_instance();
}
