...
/**
 * Redux_Travelpayouts Required Class
 *
 * @class Redux_Travelpayouts_Required
 * @version 4.0.0
...
if (! class_exists('Redux_Travelpayouts_Required', false)) {

    /**
     * Class Redux_Travelpayouts_Required
     */
    class Redux_Travelpayouts_Required extends Redux_Travelpayouts_Class
    {
        /**
        ...
        public function check_dependencies($field)
        {
            ...
            if (! empty($field['required'])) {
                if (! isset($core->required_child[ $field['id'] ])) {
                    $core->required_child[ $field['id'] ] = [];
                }

                if (! isset($core->required[ $field['id'] ])) {
                    $core->required[ $field['id'] ] = [];
                }
            ...
            switch ($operation) {
                ...
                case 'equals':
                    ...
                    if (is_array($parent_value)) {
                        foreach ($parent_value as $idx => $val) {
                            if (is_array($check_value)) {
                                foreach ($check_value as $i => $v) {
                                    if (Redux_Travelpayouts_Helpers::make_bool_str($val) === Redux_Travelpayouts_Helpers::make_bool_str($v)) {
                                        $return = true;
                                    }
                                }
                            } else {
                                if (Redux_Travelpayouts_Helpers::make_bool_str($val) === Redux_Travelpayouts_Helpers::make_bool_str($check_value)) {
                                    $return = true;
                                }
                            }
                        }
                        ...
                        if (is_array($check_value)) {
                            foreach ($check_value as $i => $v) {
                                if (Redux_Travelpayouts_Helpers::make_bool_str($parent_value) === Redux_Travelpayouts_Helpers::make_bool_str($v)) {
                                    $return = true;
                                }
                            }
                        } else {
                            if (Redux_Travelpayouts_Helpers::make_bool_str($parent_value) === Redux_Travelpayouts_Helpers::make_bool_str($check_value)) {
                                $return = true;
                            }
                        }
                    }
