<?php
/**
 * Redux_Travelpayouts Output Class
 *
 * @class   Redux_Travelpayouts_Output
 * @version 3.0.0
...
if (! class_exists('Redux_Travelpayouts_Output', false)) {

    /**
     * Class Redux_Travelpayouts_Output
     */
    class Redux_Travelpayouts_Output extends Redux_Travelpayouts_Class
    {
        /**
         * Redux_Travelpayouts_Output constructor.
         *
        ...
        public function __construct($parent)
        {
            ...
            if (empty($this->args['output_location']) || in_array('frontend', $this->args['output_location'], true)) {
                add_action('wp_head', [ $this, 'output_css' ], 150);
                add_action('wp_enqueue_scripts', [ $this, 'enqueue' ], 150);
            }

            // Login page: Maybe enqueue dynamic CSS and Google fonts.
            if (in_array('login', $this->args['output_location'], true)) {
                add_action('login_head', [ $this, 'output_css' ], 150);
                add_action('login_enqueue_scripts', [ $this, 'enqueue' ], 150);
            }

            // Admin area: Maybe enqueue dynamic CSS and Google fonts.
            if (in_array('admin', $this->args['output_location'], true)) {
                add_action('admin_head', [ $this, 'output_css' ], 150);
                add_action('admin_enqueue_scripts', [ $this, 'enqueue' ], 150);
            }

            // phpcs:ignore WordPress.NamingConventions.ValidHookName
            do_action("redux_travelpayouts/output/{$this->parent->args['opt_name']}/construct", $this);
        ...
        public function enqueue()
        {
                ...
                if (isset($section['fields'])) {
                    foreach ($section['fields'] as $fieldk => $field) {
                        if (isset($field['type']) && 'callback' !== $field['type']) {
                            $field_classes = [ 'Redux_Travelpayouts_' . $field['type'], 'TravelpayoutsSettingsFramework_' . $field['type'] ];
                            ...
                            if (! empty($core->options[ $field['id'] ]) && class_exists($field_class) && method_exists($field_class, 'output') && $this->can_output_css($core, $field)) {
                                ...
                                // phpcs:ignore WordPress.NamingConventions.ValidHookName
                                $field = apply_filters("redux_travelpayouts/field/{$core->args['opt_name']}/output_css", $field);
                            ...
                            // phpcs:ignore WordPress.NamingConventions.ValidHookName
                            do_action("redux_travelpayouts/field/{$core->args['opt_name']}/output_loop", $core, $field, $value, $style_data);

                            // phpcs:ignore WordPress.NamingConventions.ValidHookName
                            do_action("redux_travelpayouts/field/{$core->args['opt_name']}/output_loop/{$field['type']}", $core, $field, $value, $style_data);
            ...
            if (! empty($core->typography) && ! empty($core->typography) && filter_var($core->args['output'], FILTER_VALIDATE_BOOLEAN)) {
                ...
                if (! class_exists('Redux_Travelpayouts_Typography')) {
                    require_once Redux_Travelpayouts_Core::$dir . '/inc/fields/typography/class-travelpayouts-typography.php';
                }
                $typography = new Redux_Travelpayouts_Typography(null, null, $core);
                ...
                if ($core->args['async_typography'] && ! empty($core->typography)) {
                    ...
                    wp_enqueue_style('redux-google-fonts-' . $core->args['opt_name'], $typography->make_google_web_font_link($core->typography), [], $version, 'all');
        ...
        {
            if (Redux_Travelpayouts_Functions_Ex::string_starts_with($handle, 'redux-google-fonts-')) {
         ...
         *
         * @param array  $core       TravelpayoutsSettingsFramework core pointer.
         ...
         * @access      public
         */
        private function output_variables($core = [], $section = [], $field = [], $value = [], $style_data = '')
        {
         ...
         *
         * @param object $core  TravelpayoutsSettingsFramework core pointer.
        ...
        private function can_output_css($core, $field)
        {
            ...
            // phpcs:ignore WordPress.NamingConventions.ValidHookName
            $field = apply_filters("redux_travelpayouts/field/{$core->args['opt_name']}/_can_output_css", $field);
