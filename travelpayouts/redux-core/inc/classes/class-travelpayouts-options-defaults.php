...
/**
 * Redux_Travelpayouts Option Defaults Class
 *
 * @class Redux_Travelpayouts_Options_Defaults
 * @version 4.0.0
...
if (! class_exists('Redux_Travelpayouts_Options_Defaults', false)) {
    ...
    class Redux_Travelpayouts_Options_Defaults
    {
         ...
         * @return array
         */
        public function default_values($opt_name, $sections = [], $wp_data_class = null)
        {
            ...
            if (isset($_REQUEST['action'])) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
                ...
                if (Redux_Travelpayouts_Functions_Ex::string_ends_with($action, '_select2') && Redux_Travelpayouts_Functions_Ex::string_starts_with($action, 'Redux_Travelpayouts_')) {
                    return;
                }
            }
        ...
        {
            if (null === $wp_data_class && class_exists('Redux_Travelpayouts_WordPress_Data') && ! ('select' === $field['type'] && isset($field['ajax']) && $field['ajax'])) {
                $wp_data_class = new Redux_Travelpayouts_WordPress_Data($opt_name);
            }
            ...
            if (isset($field['default'])) {
                ...
                $this->options_defaults[ $field['id'] ] = apply_filters("redux_travelpayouts/{$opt_name}/field/{$field['type']}/defaults", $field['default'], $field);
