    ...
    class Redux_Travelpayouts_Options_Constructor extends Redux_Travelpayouts_Class
    {
        ...
        public function get()
        {
            ...
            if ('transient' === $core->args['database']) {
                ...
                $result = get_site_option($core->args['opt_name'], []);
            } else {
                $result = get_option($core->args['opt_name'], []);
            }
            ...
            foreach ($core->sections as $k => $section) {
                ...
                if (isset($section['fields'])) {
                    foreach ($section['fields'] as $fieldk => $field) {
                        ...
                        $core->field_types[ $field['type'] ] = isset($core->field_types[ $field['type'] ]) ? $core->field_types[ $field['type'] ] : [];
                        ...
                        if ('background' === $field['type']) {
                            if (isset($core->options[ $field['id'] ]['media']['id']) && isset($core->options[ $field['id'] ]['background-image']) && ! empty($core->options[ $field['id'] ]['background-image']) && strpos($core->options[ $field['id'] ]['background-image'], str_replace([ 'http://', 'https://' ], '', WP_CONTENT_URL)) === false) {
                        ...
                        if ('slides' === $field['type']) {
                            if (isset($core->options[ $field['id'] ]) && is_array($core->options[ $field['id'] ]) && isset($core->options[ $field['id'] ][0]['attachment_id']) && isset($core->options[ $field['id'] ][0]['image']) && ! empty($core->options[ $field['id'] ][0]['image']) && strpos($core->options[ $field['id'] ][0]['image'], str_replace([ 'http://', 'https://' ], '', WP_CONTENT_URL)) === false) {
        ...
        public function default_values()
        {
            ...
            $core->transients['changed_values'] = isset($core->transients['changed_values']) ? $core->transients['changed_values'] : [];
        ...
        public function validate_options($plugin_options)
        {
            ...
            $core->transients['changed_values'] = []; // Changed values since last save.
            ...
            unset($plugin_options['defaults'], $plugin_options['defaults_section'], $plugin_options['import'], $plugin_options['import_code'], $plugin_options['import_link'], $plugin_options['compiler'], $plugin_options['redux-section']);
            if (in_array($core->args['database'], [ 'transient', 'theme_mods', 'theme_mods_expanded' ], true)) {
