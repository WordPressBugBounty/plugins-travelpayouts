...
if (! class_exists('Redux_Travelpayouts', false)) {
    ...
    class Redux_Travelpayouts
    {
        ...
        {
            add_action('init', [ 'Redux_Travelpayouts', 'create_redux' ]);
        }
        ...
        public static function init($opt_name = '')
        {
            if (! empty($opt_name)) {
                if (! did_action('plugins_loaded')) {
                    ...
                    add_action('plugins_loaded', [ 'Redux_Travelpayouts', 'delay_init' ]);
                } else {
                    // The hook `plugins_loaded` has run, let's get going!
                    self::load_redux($opt_name);

                    remove_action('setup_theme', [ 'Redux_Travelpayouts', 'create_redux' ]);
                }
            }
        }
            ...
            if ($instance_extensions) {
                foreach ($instance_extensions as $name => $extension) {
                    ...
                    if (! isset($Redux_Travelpayouts_framework->extensions[ $name ])) {
                        $field_classes = [ $extension['class'], $old_class ];
        ...
        public static function load_redux($opt_name = '')
        {
            ...
            if (isset(self::$uses_extensions[ $opt_name ]) && ! empty(self::$uses_extensions[ $opt_name ])) {
                add_action("redux_travelpayouts/extensions/{$opt_name}/before", [ 'Redux_Travelpayouts', 'loadExtensions' ], 0);
            }
        ...
        {
            $args             = isset(self::$args[ $opt_name ]) ? self::$args[ $opt_name ] : [];
         ...
         * @deprecated No longer using camelCase naming convention.
         */
        public static function setSections($opt_name = '', $sections = []) // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
        {
            if ('' !== $opt_name) {
                Redux_Travelpayouts_Functions_Ex::record_caller($opt_name);
            }
         ...
         * @param array  $sections Section ID.
         */
        public static function set_sections($opt_name = '', $sections = [])
        {
         ...
         * @deprecated No longer using camelCase naming convention.
         */
        public static function setSection($opt_name = '', $section = []) // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
        {
            if ('' !== $opt_name) {
                Redux_Travelpayouts_Functions_Ex::record_caller($opt_name);
            }
         ...
         */
        public static function set_section($opt_name = '', $section = [], $replace = false)
        {
         ...
         */
        private static function process_field_array($opt_name = '', $section_id = '', $fields = [])
        {
         ...
         * @deprecated No longer using camelCase naming convention.
         */
        public static function setField($opt_name = '', $section_id = '', $field = []) // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
        {
            if ('' !== $opt_name) {
                Redux_Travelpayouts_Functions_Ex::record_caller($opt_name);
            }
         ...
         * @param array  $field      Field data.
         */
        public static function set_field($opt_name = '', $section_id = '', $field = [])
        {
         ...
         */
        public static function set_fields($opt_name = '', $section_id = '', $fields = [])
        {
            if (empty($fields)) {
                return;
            }
         ...
         * @deprecated No longer using camelCase naming convention.
         */
        public static function setHelpTab($opt_name = '', $tab = []) // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
        {
         ...
         * @param array  $tab      Tab data.
         */
        public static function set_help_tab($opt_name = '', $tab = [])
        {
            if (empty($tab)) {
                return;
            }
         ...
         * @return mixed
         */
        public static function get_post_meta($opt_name = '', $the_post = [], $key = '', $default = null)
        {
            self::check_opt_name($opt_name);
        ...
        {
            $data = get_file_data($file, [ 'version' ], 'plugin');
        ...
        private static function check_extension_class_file($opt_name, $name = '', $class_file = '', $instance = '')
        {
            if (file_exists($class_file)) {
                self::$uses_extensions[ $opt_name ] = isset(self::$uses_extensions[ $opt_name ]) ? self::$uses_extensions[ $opt_name ] : [];

                if (! in_array($name, self::$uses_extensions[ $opt_name ], true)) {
                    self::$uses_extensions[ $opt_name ][] = $name;
                }

                self::$extensions[ $name ] = isset(self::$extensions[ $name ]) ? self::$extensions[ $name ] : [];
         ...
         */
        public static function get_instance_extension($opt_name, $instance = [])
        {
        ...
        public static function addPsr4Extension(string $opt_name, string $className, string $extensionName = null, bool $force = false)
        {
            ...
            if (class_exists($className)) {
                ...
                if (!in_array($extensionName, self::$uses_extensions[$opt_name], true)) {
                    self::$uses_extensions[$opt_name][$extensionName] = [
                        'path'    => $classFile,
