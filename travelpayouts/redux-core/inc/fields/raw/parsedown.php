...
class Parsedown
{
            ...
            if (chop($line) === '') {
                if (isset($CurrentBlock)) {
                    $CurrentBlock['interrupted'] = (
                        isset($CurrentBlock['interrupted']) ? $CurrentBlock['interrupted'] + 1 : 1
                    );
                }
    ...
    protected function blockMarkup($Line)
    {
        ...
        if (preg_match('/^<[\/]?+(\w*)(?:[ ]*+' . $this->regexHtmlAttribute . ')*+[ ]*+(\/)?>/', $Line['text'], $matches)) {
            $element = Redux_Travelpayouts_Core::strtolower($matches[1]);
    ...
    protected function blockReference($Line)
    {
        if (strpos($Line['text'], ']') !== false
            ...
            $id = Redux_Travelpayouts_Core::strtolower($matches[1]);
    ...
    protected function lineElements($text, $nonNestables = [])
    {
        ...
        $nonNestables = (
            empty($nonNestables) ? [] : array_combine($nonNestables, $nonNestables)
        );
    ...
    protected function inlineLink($Excerpt)
    {
        ...
        if (preg_match('/^[(]\s*+((?:[^ ()]++|[(][^ )]+[)])++)(?:[ ]+("[^"]*+"|\'[^\']*+\'))?\s*+[)]/', $remainder, $matches)) {
            ...
            if (preg_match('/^\s*\[(.*?)\]/', $remainder, $matches)) {
                ...
                $definition = Redux_Travelpayouts_Core::strtolower($definition);

                $extent += strlen($matches[0]);
            } else {
                $definition = Redux_Travelpayouts_Core::strtolower($Element['handler']['argument']);
            }
    ...
    protected function elements(array $Elements)
    {
        ...
        foreach ($Elements as $Element) {
            ...
            $autoBreakNext = (
                isset($Element['autobreak']) ? $Element['autobreak'] : isset($Element['name'])
            );
    ...
    protected static function striAtStart($string, $needle)
    {
        ...
        if ($len > strlen($string)) {
            ...
            return Redux_Travelpayouts_Core::strtolower(substr($string, 0, $len)) === Redux_Travelpayouts_Core::strtolower($needle);
        }
    }
